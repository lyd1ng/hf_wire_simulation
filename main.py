...
def calculate_i(z, t):
    # First calculate the complex returning voltage
    Ur = (args.U*cmath.e**(-args.gamma * args.l) * args.r)
    I = (1.0/args.Z_L) * (args.U * cmath.e**(-args.gamma * z) - Ur * cmath.e**(args.gamma * z)) * cmath.e**(j*args.w*t)
...
parser.add_argument("-y", dest="gamma", help="Complex wire parameter", type=str)
...
parser.add_argument("--Z_L", dest="Z_L", help="Characteristic impedance", type=str)
...
parser.add_argument("-w", dest="w", help="Circular signal frequency", type=float)
parser.add_argument("-U", dest="U", default="0", help="Voltage amplitude", type=float)
...
parser.add_argument("-s", dest="number_of_samples", help="Number of samples", type=int)
parser.add_argument("--plot_u", dest="plot_u", default=True, type=bool, help="Plot voltage")
parser.add_argument("--plot_i", dest="plot_i", default=False, action="store_true", help="Plot current")
...
parser.add_argument("--gif_scale_z", dest="gif_scale_z", default="1.0", help="The z view scale of the gif")
...
while t < args.T:
    for i in range(0, args.number_of_samples):
        if args.plot_u:
            u_vectors.append(gp.gplot_vector3(i * distance_of_vectors, 0, 0, 0, 0, 0))
            u_vectors[-1].dz, u_vectors[-1].dy = calculate_u(i * distance_of_vectors, t)
        if args.plot_i:
            i_vectors.append(gp.gplot_vector3(i * distance_of_vectors, 0, 0, 0, 0, 0))
            i_vectors[-1].dz, i_vectors[-1].dy = calculate_i(i * distance_of_vectors, t)
        if args.plot_r:
            r_vectors.append(gp.gplot_vector3(i * distance_of_vectors, 0, 0, 0, 0, 0))
            r_vectors[-1].dz, r_vectors[-1].dy = calculate_r(i * distance_of_vectors)
    t = t + args.dt
